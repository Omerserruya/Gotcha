...
- "live_data": the answer was true for one moment and depends on something that must be checked - a calendar, someone's availability, current stock, a price that moves. Do NOT drop these. Rewrite the answer to state the durable part and make the dependency explicit: "When can we meet?" answered "Thursday or Friday" becomes "Viewings are usually held on Thursdays and Fridays; the exact time depends on availability and must be confirmed."
