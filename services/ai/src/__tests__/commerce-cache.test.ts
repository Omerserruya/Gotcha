...
describe("25. webhook-driven invalidation", () => {
  ...
  it("different viewer permissions do NOT share a cache entry (capabilities are per-viewer)", async () => {
    const reader = { canRead: true, canOpen: true, canCancel: false, canRefund: false, canTag: false, canNote: false };
    const manager = { canRead: true, canOpen: true, canCancel: true, canRefund: true, canTag: true, canNote: true };
