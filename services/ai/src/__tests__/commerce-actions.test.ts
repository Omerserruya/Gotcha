...
describe("an action the provider cannot execute is refused, not attempted", () => {
  it("rejects resend_confirmation as an unknown action", async () => {
    // It was removed once the adapter proved its handler always throws.
    const res: any = await executeCommerceAction(
