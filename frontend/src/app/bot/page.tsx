    ...
    return (
      <div className="max-w-3xl mx-auto">
        <div className="bg-white rounded-2xl shadow-card border border-gray-100 p-8 text-center">
          ...
          <h4 className="font-semibold text-gray-900 mb-2">{t("bot.notConfiguredTitle")}</h4>
          <p className="text-sm text-gray-500 max-w-sm mx-auto">
            {t("bot.notConfiguredBody")}
          </p>
        </div>
      </div>
    );
    ...
    <div className="max-w-3xl space-y-6">
      ...
      <div className="bg-gradient-to-br from-emerald-500 to-teal-600 rounded-2xl p-5 text-white shadow-lg">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            <h3 className="font-bold text-lg">{t("bot.title")}</h3>
            <p className="text-white/80 text-sm">{t("bot.subtitle")}</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-card border border-gray-100 p-5">
        <div className="flex items-center justify-between">
          <div>
            <h4 className="font-semibold text-gray-900">{t("bot.active")}</h4>
            <p className="text-sm text-gray-400 mt-0.5">{t("bot.activeDesc")}</p>
          </div>
          <button
      ...
      {/* System Prompt */}
      <div className="bg-white rounded-2xl shadow-card border border-gray-100 p-5">
        <h4 className="font-semibold text-gray-900 mb-1">{t("bot.systemPrompt")}</h4>
        <p className="text-sm text-gray-400 mb-3">{t("bot.systemPromptDesc")}</p>
        <textarea
          value={systemPrompt}
      ...
      <div className="bg-white rounded-2xl shadow-card border border-gray-100 p-5">
        <h4 className="font-semibold text-gray-900 mb-1">{t("bot.autonomousBehavior")}</h4>
        <p className="text-sm text-gray-400 mb-4">{t("bot.autonomousBehaviorDesc")}</p>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">{t("bot.maxMessages")}</label>
            ...
            <p className="text-[11px] text-gray-400 mt-1">{t("bot.maxMessagesHint")}</p>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">{t("bot.maxMinutes")}</label>
            <input type="number" value={maxAutonomousMinutes} onChange={(e) => setMaxAutonomousMinutes(Math.min(60, Math.max(1, parseInt(e.target.value) || 1)))} min={1} max={60}
              className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-emerald-200 focus:border-emerald-300 focus:bg-white outline-none transition" />
            <p className="text-[11px] text-gray-400 mt-1">{t("bot.maxMinutesHint")}</p>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">{t("bot.confidenceThreshold")}: {confidenceThreshold.toFixed(2)}</label>
            ...
            <p className="text-[11px] text-gray-400 mt-1">{t("bot.confidenceHint")}</p>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">{t("bot.escalationMessage")}</label>
            <textarea value={escalationMessage} onChange={(e) => setEscalationMessage(e.target.value)} placeholder={t("bot.escalationPlaceholder")} rows={3}
      ...
      <div className="bg-white rounded-2xl shadow-card border border-gray-100 p-5">
        <div className="flex items-center justify-between mb-1">
          <h4 className="font-semibold text-gray-900">{t("bot.rules")}</h4>
        ...
        <p className="text-sm text-gray-400 mb-3">{t("bot.rulesDesc")}</p>
        <div className="space-y-2">
          {rules.length === 0 ? (
            <p className="text-sm text-gray-300 italic py-3 text-center">{t("bot.noRules")}</p>
          ) : rules.map((rule, i) => (
            <div key={i} className="flex items-center gap-2">
              <span className="text-xs text-gray-400 font-mono w-5 shrink-0 text-end">{i + 1}</span>
              <input type="text" value={rule} onChange={(e) => { const u = [...rules]; u[i] = e.target.value; setRules(u); }} placeholder={t("bot.rulePlaceholder")}
      ...
      {/* Model Settings */}
      <div className="bg-white rounded-2xl shadow-card border border-gray-100 p-5">
        <h4 className="font-semibold text-gray-900 mb-3">{t("bot.modelSettings")}</h4>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">{t("bot.provider")}</label>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">{t("bot.model")}</label>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">{t("bot.temperature")}: {temperature.toFixed(1)}</label>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">{t("bot.maxTokens")}</label>
      ...
      <div className="flex items-center gap-3 pb-6">
        <button onClick={handleSave} disabled={loading}
          ...
          {loading ? t("common.loading") : t("bot.save")}
        </button>
        {saved && (
      ...
      <AppLayout>
        <div className="p-3 md:p-6 overflow-y-auto h-screen">
          <div className="flex items-center justify-between mb-4 md:mb-6">
            <h1 className="text-xl md:text-2xl font-bold text-gray-900">{t("bot.pageTitle")}</h1>
          </div>
