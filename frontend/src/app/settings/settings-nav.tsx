// The Settings navigation definition, in its own module so (a) the layout
// file keeps Next.js's restricted export surface and (b) tests can assert
// the IA invariants (no Policies/Tools/generic-Integrations entries, the
// Account/Workspace grouping, one canonical route per concept).
...
// The Settings IA: strict responsibility isolation. Settings owns workspace
// and business configuration; AI Studio owns everything about AI employees
// (tools, HITL, execution policies). Removed entries (Policies, Tools, the
// generic Integrations, standalone Users/Departments/Voice-channels lists)
// live on as redirects - see lib/settings-routes.ts.
// Exported for tests (settings-nav definition invariants).
export const settingsNav: SettingsNavItem[] = [
  // ── Account (personal + security) ──
  { href: "/settings/account", labelKey: "settings.nav.account", icon: AccountIcon, group: "personal" },
  // Workspace security policy (MFA enforcement + compliance). Admin-only.
  { href: "/settings/security", labelKey: "settings.nav.security", icon: SecurityIcon, adminOnly: true, group: "personal" },
  // ── Workspace ──
  ...
  // Source of Truth / business data systems (NOT a tool marketplace).
  { href: "/settings/business-systems", labelKey: "settings.nav.businessSystems", icon: IntegrationsIcon },
  { href: "/settings/notifications", labelKey: "notifications.title", icon: NotificationsIcon },
  // Kept: real workspace-level configuration with no other canonical home.
