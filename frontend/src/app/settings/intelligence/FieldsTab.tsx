...
const SCOPE_META: Record<string, { en: string; he: string; cls: string }> = {
  ...
  review_required: { en: "Needs scope review", he: "דורש בחירת תחום", cls: "bg-amber-100 text-amber-800 border-amber-300" },
};
...
const STRICTNESS_PRESETS = [
  { value: 0.5, en: "Capture more", he: "לכידה רחבה" },
  { value: 0.7, en: "Balanced", he: "מאוזן" },
  { value: 0.85, en: "Only when certain", he: "רק כשבטוח" },
] as const;

function thresholdToPreset(threshold: number | null | undefined): number {
  if (threshold == null) return 0.7;
  const dists = STRICTNESS_PRESETS.map((p) => ({ v: p.value, d: Math.abs(p.value - threshold) }));
...
function FieldRow({
  ...
  const scopeMeta = SCOPE_META[field.scope] ?? SCOPE_META.customer!;
  const isReviewRequired = field.scope === "review_required";
  return (
    <div className="flex items-center gap-3 py-2 px-2 rounded-lg hover:bg-gray-50 group">
      <span className={clsx("text-[10px] font-medium px-2 py-0.5 rounded-full border shrink-0", scopeMeta.cls)}>
        {he ? scopeMeta.he : scopeMeta.en}
      </span>
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          ...
          {isReviewRequired && (
            <span className="text-[10px] px-1.5 py-0.5 rounded-full border bg-amber-50 text-amber-700 border-amber-300 shrink-0">
              {he ? "דורש בחירת תחום" : "Needs scope review"}
            </span>
          )}
        </div>
...
function FieldEditor({
  ...
  const [draft, setDraft] = useState<FieldDefinitionInput>({
    ...emptyDraft(),
    ...initial,
    confidenceThreshold: thresholdToPreset((initial as any).confidenceThreshold),
  });
  const [optionInput, setOptionInput] = useState("");
  const [exampleInput, setExampleInput] = useState("");
  const [negExampleInput, setNegExampleInput] = useState("");
  ...
  function addExample() {
    ...
    if (!ex.includes(v)) set("examples", [...ex, v]);
    setExampleInput("");
  }

  function addNegExample() {
    const v = negExampleInput.trim();
    if (!v) return;
    const neg = draft.negativeExamples ?? [];
    if (!neg.includes(v)) set("negativeExamples", [...neg, v]);
    setNegExampleInput("");
  }
  ...
  const chipInputBlock = (
    ...
    <div>
      <div className="flex flex-wrap gap-1.5 mb-1">
        {items.map((item) => (
          <span key={item} className="inline-flex items-center gap-1 text-xs bg-white border border-gray-200 rounded-full px-2 py-0.5">
            {item}
            <button onClick={() => onRemove(item)} className="text-gray-400 hover:text-red-600">×</button>
          </span>
        ))}
      </div>
      <div className="flex gap-2">
        ...
        />
        <button onClick={onAdd} className="px-3 py-2 text-sm bg-gray-100 hover:bg-gray-200 rounded-lg">
          {he ? "הוסף" : "Add"}
        </button>
      </div>
    </div>
    ...
    <div className="border border-violet-200 bg-violet-50/30 rounded-lg p-4 space-y-3">
      ...
      {isEnum && (
        <div className="flex flex-col gap-1">
          ...
          {chipInputBlock(
            ...
            setOptionInput,
            addOption,
            (item) => set("options", (draft.options ?? []).filter((x) => x !== item)),
            he ? "הוסיפו אפשרות ו-Enter" : "Type an option, press Enter",
          )}
        </div>
      )}
      ...
      <div className="flex flex-col gap-1">
        <span className="text-xs text-gray-600">
          {he ? "דוגמאות (עוזר ל-AI לזהות)" : "Examples (help the AI recognize this)"}
        </span>
        {chipInputBlock(
          draft.examples ?? [],
          exampleInput,
          setExampleInput,
          addExample,
          (item) => set("examples", (draft.examples ?? []).filter((x) => x !== item)),
          he ? "לדוגמה: כ-300 אורחים" : "e.g. around 300 guests",
        )}
      </div>
      ...
      <div>
        ...
        {showAdvanced && (
          <div className="mt-3 space-y-3 border-t border-gray-100 pt-3">
            <div className="flex flex-col gap-1">
              <span className="text-xs text-gray-600">
                {he ? "דוגמאות נגד (לא חובה)" : "Counter-examples (optional)"}
              </span>
              {chipInputBlock(
                ...
                setNegExampleInput,
                addNegExample,
                (item) => set("negativeExamples", (draft.negativeExamples ?? []).filter((x) => x !== item)),
                he ? "ערך שאינו דוגמה טובה" : "Value that is NOT a good match",
              )}
            </div>

            <div className="flex flex-col gap-1">
              <span className="text-xs text-gray-600">
                {he ? "רגישות חילוץ" : "Extraction strictness"}
              </span>
              <div className="flex rounded-lg overflow-hidden border border-gray-200 w-fit">
                {STRICTNESS_PRESETS.map((preset) => {
                  const active = draft.confidenceThreshold === preset.value;
                  return (
                    <button
                      key={preset.value}
                      type="button"
                      onClick={() => set("confidenceThreshold", preset.value)}
                      className={clsx(
                        "px-3 py-1.5 text-xs font-medium transition border-r border-gray-200 last:border-r-0",
                        active
                          ? "bg-violet-600 text-white"
                          : "bg-white text-gray-600 hover:bg-gray-50",
                      )}
                    >
                      {he ? preset.he : preset.en}
                    </button>
                  );
                })}
              </div>
