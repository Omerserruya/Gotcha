...
fs.writeFileSync(path.join(OUT_DIR, 'pseudo.css'),
  '/* GENERATED by tools/dc2jsx.mjs - style-<pseudo> attributes from the design. */\n' +
  pseudoRules.join('\n') + '\n');
fs.writeFileSync(path.join(OUT_DIR, 'helmet.html'), helmet);

console.log('Template.jsx :', tsx.split('\n').length, 'lines');
console.log('pseudo.css   :', pseudoRules.length, 'rules');
console.log('helmet.html  :', helmet.length, 'bytes');
if (warnings.length) {
  console.log('\nwarnings:');
